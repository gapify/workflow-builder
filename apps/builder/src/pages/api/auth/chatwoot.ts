...
export default async function customAuthHandler(
  ...
  try {
    ...
    if (!accessToken) {
      return res.status(400).json({ error: 'No access_token found in decoded data' })
    }

    // 3. Verify the token with your external API
    //    Replace with your actual verification endpoint
    const verificationResponse = await fetch(env.CHATWOOT_API_URL+'/auth/validate_token', {
      method: 'GET',
      headers: {
    ...
    const existingUser = await prisma.user.findFirst({
      where: {
        email: 'chatwoot-account-' + verificationData.payload.data.account_id +'@chatwoot.com'
      }
    }) as User;
